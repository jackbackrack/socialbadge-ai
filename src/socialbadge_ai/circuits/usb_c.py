...
class USB_C(Circuit):
    ...
    def __init__(self):
        ...
        self.r_cc1 = Resistor(resistance=5.1e3 * ohm)
        self.r_cc1_nets = [self.r_cc1.p1 + self.j1.CC1, self.r_cc1.p2 + self.GND]

        self.r_cc2 = Resistor(resistance=5.1e3 * ohm)
        self.r_cc2_nets = [self.r_cc2.p1 + self.j1.CC2, self.r_cc2.p2 + self.GND]

        # 27 Ω series resistors on USB data lines (RP2040 datasheet requirement)
        self.r_dp = Resistor(resistance=27.0 * ohm)
        self.r_dp_nets = [self.r_dp.p1 + self.USB_DP, self.r_dp.p2 + self.DP]

        self.r_dn = Resistor(resistance=27.0 * ohm)
        self.r_dn_nets = [self.r_dn.p1 + self.USB_DN, self.r_dn.p2 + self.DN]
